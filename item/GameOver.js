export default class Gameover {
  ...
  update() {
    ...
    if (
      this.continueImage.ylocate > 250 &&
      Math.abs(this.initFallSpeed) <= 0.3
    ) {
  ...
  notifyClcikedYesBtn(x, y) {
    ...
    if (
      xdir < this.btnYes.width &&
      xdir > 0 &&
      ydir < this.btnYes.height &&
      ydir > 0
    ) {
      //Yes Btn 눌렀을 때
  ...
  notifyClcikedNoBtn(x, y) {
    ...
    if (
      xdir < this.btnNo.width &&
      xdir > 0 &&
      ydir < this.btnNo.height &&
      ydir > 0
    ) {
      //No Btn 눌렀을 때
